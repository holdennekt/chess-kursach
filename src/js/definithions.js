...
const createCells = block => {                //defining divs in container
    let light = 0;
    const divs = [[], [], [], [], [], [], [], []];
    for (let i = 0; i < 8; i++) {
        light ^= 1;
...
const clickedOnCell = click => {
    const i = parseInt(click.target.id[5]), j = parseInt(click.target.id[6]);
